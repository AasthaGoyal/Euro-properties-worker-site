...
const AddTimesheet = (props) => {
	...
	const [currentJobsite, setCurrentJobsite] = useState([]);
	...
	useEffect(() => {
		...
		axios.get(`${BASE_URL}/api/users`).then((res) => {
			...
			axios.get(`${BASE_URL}/api/workerpayslips`).then((response) => {
				...
				let filterPayslips = _.filter(payslips, ['employee', filterEmployee[0]._id]);
	...
	const punchIn = (e) => {
		...
		axios
			.post(`${BASE_URL}/api/employeeTimesheets/addemployeeTimesheet`, punchInObj)
			.then((res) => {
	...
	const pause = (e) => {
		...
		axios.post(`${BASE_URL}/api/employeeBreaks/addemployeeBreak`, pauseObj).then((res) => {
	...
	const resume = (e) => {
		...
		axios.post(`${BASE_URL}/api/employeeBreaks/addemployeeBreak`, resumeObj).then((res) => {
			...
			axios.get(`${BASE_URL}/api/employeeBreaks`).then((res) => {
				...
				axios
					.post(`${BASE_URL}/api/employeeBreakFinal/addemployeeBreakFinal`, breakObject)
					.then((response) => {
				...
				axios
					.put(
						`${BASE_URL}/api/employeeBreaks/edit/${filterAttendance[0]._id}`,
						pauseObject,
					)
					.then((res) => {
						axios.get(`${BASE_URL}/api/employeeBreaks`).then((res) => {
	...
	const punchOut = (e) => {
		...
		axios
			.post(`${BASE_URL}/api/employeeTimesheets/addemployeeTimesheet`, punchOutObj)
			.then((res) => {
	...
	const onSelectJobSite = (e) => {
		...
		_.map(jobsites, (jobsite) => {
			if (jobsite._id == jobsiteId) {
				...
				setCurrentJobsite(jobsite.address);
			}
		});
	...
	const searchJob = (e) => {
		axios.get(`${BASE_URL}/api/jobsites`).then((res) => {
			...
			console.log('serach item', search);
								...
								<div className='row align-items-end'>
									<div className='col-lg-6'>
										...
										<span className='display-6 fw-bold text-success'>
											{' '}
											{`${timeResult.Days} Days ${timeResult.Hours} Hrs  ${timeResult.Minutes} Mins.`}
										</span>
									</div>
									{/* <div className='col-lg-6'>
                  <Chart

	...
	if (currentTasks.length === 0) {
		...
		taskTable = _.map(currentTasks, (task) => {
			...
			return (
				<tr key={task._id}>
					<td>
						<Link to={`/task-pages/EditTasks/${task._id}`}>{task.taskTitle}</Link>
					</td>
					<td>{jobsiteList}</td>
					<td>
						{' '}
						{finalusers.map((worker) => {
							return (
					...
					<td>
						<Link to={`/task-pages/EditTasks/${task._id}`}>
							<Button
	...
	const searchTasks = (e) => {
		...
		axios.get(`${BASE_URL}/api/tasks`).then((res) => {
			...
			var taskResult = _.filter(taskData, function (obj) {
				return obj.taskTitle.indexOf(search) !== -1;
			});
	...
	return (
		<PageWrapper title='Jobsites'>
			<SubHeader>
				...
				<SubHeaderRight>
					{filteredJobsite && <CommonFilterTag title='Jobsite' text={filteredJobsite} />}
					<SubheaderSeparator />
					<Dropdown>
